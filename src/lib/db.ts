...
if (typeof window === "undefined" && dbSource === "pglite") {
  globalBoot.__pgBootstrapPromise__ ??= ensureDbReady().catch((err) => {
    ...
    // Do not rethrow: this promise is not awaited, and a rejection here
    // kills the Node isolate (every production request 500s) even for
    // visitors who never touch the database.
  });
}
