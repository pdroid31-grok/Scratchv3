...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    ...
    onSuccess(ctx) {
      const token = (ctx.data as { session?: { token?: string } } | null)?.session?.token;
...
// Keep the session token in localStorage so a closed tab (and the Grok iframe)
// does not force a fresh sign-in. Cookie auth still works when the browser
// sends it; this bearer is the backup those cookies miss.
